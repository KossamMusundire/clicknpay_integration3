...
def get_settings():
    ...
    return {"public_id": public_id, "create_url": CREATE_URL, "status_url": STATUS_URL}
...
def create_or_update_log(order_id, reference, amount=0, status="PENDING", payme_url=None, raw=None, payment_entry=None, pr_name=None, source="CyteERP Portal", customer=None, email=None):
    try:
        if not order_id: order_id = reference
        if frappe.db.exists("ClicknPay Transaction Log", order_id):
            ...
            doc = frappe.get_doc({
                ...
                "order_id": order_id, "reference": reference, "status": status, "amount": amount,
                "customer": customer, "customer_email": email, "payme_url": payme_url or "",
                "raw_response": json.dumps(raw, indent=2)[:10000] if raw else "",
                "creation_time": frappe.utils.now(), "payment_entry": payment_entry,
                "payment_request": pr_name, "sales_invoice": reference if frappe.db.exists("Sales Invoice", reference) else None,
...
def initiate_payment(reference=None, subscription=None, email=None, phone=None, plan_name=None, qty=1, description=None, return_url=None, currency=None, payment_request=None):
    ...
    if not pr_name:
        existing_pr = frappe.get_all("Payment Request", filters={"reference_name": reference, "docstatus": ["<", 2]}, limit=1)
        if existing_pr: pr_name = existing_pr[0].name

    # GW Keys Subscription
    if subscription_arg and frappe.db.exists("Subscription", subscription_arg):
        ...
        if outs: invoice_name = outs[0].name

    if not invoice_name and reference and frappe.db.exists("Sales Invoice", reference):
        invoice_name = reference
    elif not invoice_name and reference and frappe.db.exists("Subscription", reference):
    ...
    # ===== USE CASE 1 FIX: Email search for join-gw-keys =====
    # This is what Version 2 was missing - KEEP THIS!
    ...
    if not invoice_name and search_email:
        ...
        if inv: invoice_name = inv[0].name

    if not invoice_name:
        frappe.throw(f"Invoice not found Ref: {reference} Email: {search_email}")
    ...
    is_gw_keys = bool(inv_doc.subscription) or subscription_arg # GW Keys has subscription
    source = "GW Keys Subscription" if is_gw_keys else "CyteERP Portal"

    # ===== DUAL RETURN URL =====
    # GW Keys: redirect back to join-gw-keys page (your client_script handleClicknPayReturn expects status=success on same page)
    # CyteERP: redirect to payment-success with doctype/docname to avoid 500
    if not return_url:
        if is_gw_keys:
            return_url = f"{site_url}/join-gw-keys?status=success&invoice={invoice_name}&gateway=clicknpay&gateway_status=SUCCESS"
        else:
    ...
    payload = {
        "channel": "AUTOMATED", "clientReference": invoice_name, "currency": curr,
        "customerCharged": True, "customerPhoneNumber": phone.replace(" ", ""),
        "description": (description or f"Payment x{qty}")[:200], "multiplePayments": False,
        "orderYpe": "DYNAMIC", "productsList": products,
        "publicUniqueId": settings["public_id"], "returnUrl": return_url
    }
    ...
    create_or_update_log(order_id=order_id, reference=invoice_name, amount=inv_doc.grand_total, status="PENDING", payme_url=pay_url, raw=j, pr_name=pr_name, source=source, customer=inv_doc.customer, email=email)
...
def pay_invoice(invoice_name=None):
    ...
    if not invoice_name: frappe.throw("Missing invoice_name")
...
def clicknpay_callback():
            ...
            if frappe.db.exists("Sales Invoice", ref):
                ...
                if inv.outstanding_amount > 0:
                    if not frappe.get_all("Payment Entry", filters={"reference_no": ref, "docstatus": ["<", 2]}, limit=1):
                        pe = frappe.get_doc({
                            "doctype": "Payment Entry", "payment_type": "Receive", "party_type": "Customer", "party": inv.customer,
                            "posting_date": today(), "paid_amount": inv.grand_total, "received_amount": inv.grand_total,
                            "reference_no": ref, "reference_date": today(), "mode_of_payment": "ClicknPay",
    ...
    # ===== FINAL REDIRECT THAT WORKS FOR BOTH =====
    # Contains: doctype/docname (fixes 500) + invoice/status/gateway (fixes join-gw-keys popup)
    frappe.local.response["type"] = "redirect"
    frappe.local.response["location"] = f"{get_url()}/payment-success?doctype=Sales Invoice&docname={ref}&invoice={ref}&status={status_val}&gateway=clicknpay&gateway_status={status_val}"
